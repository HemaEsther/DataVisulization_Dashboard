...
export default function IntensityChart({ data }) {
  ...
  // Aggregate intensity by topic (top 10)
  const topicIntensity = data.reduce((acc, item) => {
    const topic = item.topic || "Unknown";
    if (!acc[topic]) {
      acc[topic] = { topic, intensity: 0, count: 0 };
    }
  ...
  const chartData = Object.values(topicIntensity)
    .map((item) => ({
      label: item.topic.length > 15 ? `${item.topic.substring(0, 15)}...` : item.topic,
  ...
  return (
    ...
    >
      ...
      >
        Average Intensity by Topic (Top 10)
      </h3>
