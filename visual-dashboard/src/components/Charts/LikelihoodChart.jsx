...
export default function LikelihoodChart({ data }) {
  ...
  // Aggregate likelihood by region (top 12)
  const regionLikelihood = data.reduce((acc, item) => {
    const region = item.region || "Unknown";
    if (!acc[region]) {
      acc[region] = { region, likelihood: 0, count: 0 };
    }
    acc[region].likelihood += item.likelihood || 0;
    acc[region].count += 1;
    return acc;
  }, {});

  const chartData = Object.values(regionLikelihood)
    .map((item) => ({
      label: item.region.length > 15 ? `${item.region.substring(0, 15)}...` : item.region,
  ...
  return (
    ...
    >
      ...
      >
        Average Likelihood by Region (Top 12)
      </h3>
      <ResponsiveContainer width="100%" height={280}>
        <LineChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 60 }}>
          ...
          <Line
            ...
            strokeWidth={3}
            dot={{ fill: isDark ? "#10B981" : "#10B981", r: 4 }}
            activeDot={{ r: 6 }}
          />
        </LineChart>
      </ResponsiveContainer>
